...
class MongoService {
  ...
  prepareDocumentParts(asyncAPIData) {
    ...
    const parsedOriginal =
      asyncAPIData.parsed ??
      asyncAPIData.originalParsed ??
      null;

    const originalDocument = this.prepareOriginalDocument(
      originalContent,
      parsedOriginal,
      asyncAPIData.originalFormat || asyncAPIData.format
    );
  ...
  /**
   * Prepare document for originals collection including both raw and parsed forms
   * @param {string|Object|null} originalContent - Raw AsyncAPI content
   * @param {Object|null} parsedOriginal - Parsed AsyncAPI object
   * @param {string|undefined} formatHint - Optional format hint
   * @returns {Object|null} Prepared original document payload
   */
  prepareOriginalDocument(originalContent, parsedOriginal, formatHint) {
    const hasRawString = typeof originalContent === 'string' && originalContent.trim().length > 0;
    const hasObjectContent =
      originalContent && typeof originalContent === 'object' && !Array.isArray(originalContent);
    const hasParsed = parsedOriginal && typeof parsedOriginal === 'object';

    if (!hasRawString && !hasObjectContent && !hasParsed) {
      return null;
    }

    const rawContent = hasRawString
      ? originalContent
      : hasObjectContent
        ? JSON.stringify(originalContent, null, 2)
        : hasParsed
          ? JSON.stringify(parsedOriginal, null, 2)
          : null;

    const documentContent = hasParsed
      ? parsedOriginal
      : hasObjectContent
        ? originalContent
        : this.tryParseOriginalRaw(originalContent);

    const format = formatHint || this.detectOriginalFormat(originalContent);
  ...
  /**
   * Attempt to parse raw original content if it's JSON
   * @param {string|Object|null} rawContent - Raw content passed during insertion
   * @returns {Object|null} Parsed object or null when parsing fails
   */
  tryParseOriginalRaw(rawContent) {
  ...
  /**
   * Detect original document format based on content
   * @param {string|Object|null} rawContent - Raw content passed during insertion
   * @returns {string} Detected format string
   */
  detectOriginalFormat(rawContent) {
    if (!rawContent) {
      return 'unknown';
    }
    ...
    try {
      ...
      if (original) {
        ...
        if (original.document) {
          originalDocument.document = original.document;
        }
