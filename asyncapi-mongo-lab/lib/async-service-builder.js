    ...
    const names = parameters
      .map((parameter) => {
        ...
        if (parameter && typeof parameter === 'object') {
          return cleanStr(parameter.name);
        }
        return null;
      })
      .filter(Boolean);
...
const extractMessages = (spec) => {
  ...
  operations.forEach((operation) => {
    const messages = asArray(operation?.messages);
    messages.forEach((message, index) => {
      const fallbackName = message?.name || operation?.operationId || operation?.channel || `message-${index + 1}`;
      addMessage(message, fallbackName);
    });
  });
...
const buildAsyncService = (spec = {}, explicitId) => {
  ...
  const service = {
    ...
    asyncapiVersion,
    termsOfService: cleanStr(info.termsOfService) ?? null,
    contactName: cleanStr(info.contact?.name) ?? null,
    contactEmail: cleanStr(info.contact?.email) ?? null,
    contactUrl: cleanStr(info.contact?.url) ?? null,
    licenseName: cleanStr(info.license?.name) ?? null,
    licenseUrl: cleanStr(info.license?.url) ?? null,
    externalDocsDescription: cleanStr(spec.externalDocs?.description) ?? null,
    externalDocsUrl: cleanStr(spec.externalDocs?.url) ?? null,
