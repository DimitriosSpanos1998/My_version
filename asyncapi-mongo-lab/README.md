...
- **Original** (`ORIGINAL_COLLECTION_NAME`): Stores the raw AsyncAPI source content and its parsed representation (when available)
  for auditing or re-processing purposes.
...
const result = await mongoService.insertAsyncAPIDocument({
  ...
  parsed: asyncAPISpec, // optional: include to persist structured copy in originals collection
